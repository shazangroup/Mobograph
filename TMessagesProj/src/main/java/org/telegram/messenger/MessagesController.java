...
public class MessagesController implements NotificationCenter.NotificationCenterDelegate {
    ...
    public MessagesController() {
        ...
        //Customized:
        SharedPreferences Prefs = ApplicationLoader.applicationContext.getSharedPreferences("TelegramConfig", Activity.MODE_PRIVATE);
        ...
        if (first) {
            ...
            if (dialog != null) {
                ...
                if (onlyHistory == 0 || onlyHistory == 3) {
                    ...
                    dialogsSpecialHides.remove(dialog);
    ...
    public void loadDialogs(final int offset, final int count, boolean fromCache) {
        if (loadingDialogs) {
            Log.i("finalsoft-da", "return loadDialogs");
            return;
        }
        Log.i("finalsoft-da", "execute loadDialogs");
        loadingDialogs = true;
        NotificationCenter.getInstance().postNotificationName(NotificationCenter.dialogsNeedReload);
                ...
                if (loadType == 1 && dialogsRes.dialogs.size() == 0) {
                    AndroidUtilities.runOnUIThread(new Runnable() {
                        ...
                        public void run() {
                            ...
                            Log.i("finalsoft-da", "loadDialogs(0, count, false);");
                            loadingDialogs = false;
                            if (resetEnd) {
                                dialogsEndReached = false;
                                serverDialogsEndReached = false;
    ...
    public void markDialogAsRead(final long dialog_id, final int max_id, final int max_positive_id, final int max_date, final boolean was, final boolean popup) {
        ...
        boolean dontTik = sharedPreferences.getBoolean("dontTik", true);
...
//    }
    ...
    public void sortDialogs(HashMap<Integer, TLRPC.Chat> chatsDict) {
        ...
        for (int a = 0; a < dialogs.size(); a++) {
            ...
            int hide = hideDBAdapter.isHidden((int) d.id);
            if (hide != -1) {
                if (hide == 0123) {
                    dialogsHides.add(d);
                } else if (hide == 0100) {
                ...
                if (hideChannelController.is(d.id)) {
                    try {
                        a--;
                        dialogsServerOnly.remove(d);
                        dialogs.remove(d);
                        dialogsGroupsOnly.remove(d);
                        dialogsGroups.remove(d);
                        dialogsGroupsAll.remove(d);
                        dialogsFavs.add(d);
                        dialogsChannels.remove(d);
                        dialogsBots.remove(d);
                        continue;
                    } catch (Exception e) {
                ...
                public void run(final TLObject response, final TLRPC.TL_error error) {
                    AndroidUtilities.runOnUIThread(new Runnable() {
                        ...
                        public void run() {
                            ...
                            Log.i("finalsoft", "mc : " + response.getClass().getName().toString());
                            if (error == null) {
